...
# If finetune InfoNeRF model, comment out function below
# def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
...
def create_nerf(args):
    ...
    if not args.render_only and not args.stop_wandb_logging:
        wandb.watch(model)
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        try:
            ...
            print("cant load optimizer parameter. Skip...")
...
def config_parser():
    ...
    parser.add_argument("--wandb_project",   type=str, default='BMVC2023')
    ...
    parser.add_argument("--render_poses", "--consistency_poses",
        type=str, choices=['loaded', 'interpolate_train_all', 'uniform', 'manifold'], default='loaded')
    ...
    # ManifoldNerf arguments
    parser.add_argument("--distance_threshold", type=float, default=2.5)
    parser.add_argument("--hardcode_manifold_pair", type=str, default=None)
    
    # wandb arguments
    parser.add_argument("--stop_wandb_logging", action="store_true")
...
def train():
    ...
    # refer from InfoNeRF: 
        ...
        images, poses, hwf, masks, radius, sphere_center = load_dtu_data(args.datadir)
        render_poses = poses
        print('Loaded DTU', images.shape, poses.shape, hwf, args.datadir)
        
        np.random.seed(args.seed)
        i_train = np.random.choice(list(range(len(poses))),args.max_train_views, replace=False)
        i_test = [i for i in range(len(poses)) if (i not in i_train)]
        i_val = i_test
    ...
    # Get manifold pair
    if args.hardcode_manifold_pair is not None:
        manifold_pair = eval(args.hardcode_manifold_pair)
    else:
        manifold_pair = []
        if args.dataset_type == 'blender':
            for idx_1 in range(i_train.shape[0]):
                for idx_2 in range(idx_1+1, i_train.shape[0]):
                    pose1 = poses[i_train[idx_1], :3, 3]
                    pose2 = poses[i_train[idx_2], :3, 3]
                    center_point = ( pose1 + pose2 ) / 2.0
                    distance = np.sqrt(np.sum((center_point) ** 2))
                    if distance > args.distance_threshold: # 原点とカメラの距離: 4.0
                        print(f"Manifold pair {i_train[idx_1]} -> {i_train[idx_2]} distamce: {distance}")
                        manifold_pair.append([i_train[idx_1], i_train[idx_2]])
        elif args.dataset_type == 'dtu':
            for idx_1 in range(i_train.shape[0]):
                for idx_2 in range(idx_1+1, i_train.shape[0]):
                    pose1 = poses[i_train[idx_1], :3, 3]
                    pose2 = poses[i_train[idx_2], :3, 3]
                    center_point = ( pose1 + pose2 ) / 2.0
                    distance = np.sqrt(np.sum((center_point - np.array(sphere_center) ) ** 2))
                    print(distance)
                    if distance > args.distance_threshold: # 原点とカメラの距離: 約2.0
                        print(f"Manifold pair {i_train[idx_1]} -> {i_train[idx_2]} distamce: {distance}")
                        manifold_pair.append([i_train[idx_1], i_train[idx_2]])
    ...
    for i in trange(start, N_iters):
        ...
        if any_rendered_loss and render_loss_iter:
            with torch.no_grad():
                ...
                if args.render_poses == 'manifold':
                    assert args.dataset_type == 'blender' or args.dataset_type == 'dtu'
                    # select 2 pose randomly
                    poses_i = manifold_pair[np.random.randint(len(manifold_pair))]
                    pose1 = poses[poses_i[0], :3, :4].cpu()
                    pose2 = poses[poses_i[1], :3, :4].cpu()
                    s12  = np.random.uniform(*args.render_poses_interpolate_range)
                    if args.dataset_type == 'blender':
                        pose = geometry.interp_pose(pose1, pose2, s12)  
                    elif args.dataset_type == 'dtu':
                        pose = geometry.interp_pose(pose1, pose2, s12, center=sphere_center)
                    target_pseudo_emb = target_embeddings[np.where(i_train==poses_i[0])[0], 0] * (1-s12) + \
                                        target_embeddings[np.where(i_train==poses_i[1])[0], 0] * s12 
                else:
                    raise NotImplementedError("This code is only for ManifoldNeRF.\n")
        ...
        if calc_ctr_loss and render_loss_iter:
            ...
            if args.consistency_model_type.startswith('clip_vit'):
                ...
                rendered_emb = rendered_embedding
                if args.N_importance > 0:
                    rendered_emb0 = rendered_embedding0

            # Sample a single random target for consistency loss
            # Changed from original
            consistency_loss = torch.cosine_similarity(target_pseudo_emb, rendered_emb, dim=-1)
            if args.N_importance > 0:
                consistency_loss0 = torch.cosine_similarity(target_pseudo_emb, rendered_emb0, dim=-1)

            loss = loss + (1 - consistency_loss) * args.consistency_loss_lam
            if args.N_importance > 0:
                loss = loss + (1 - consistency_loss0) * args.consistency_loss_lam0
        ...
        if i%args.i_log_raw_hist==0 and not args.render_only and not args.stop_wandb_logging:
        ...
        if metrics and not args.stop_wandb_logging:
